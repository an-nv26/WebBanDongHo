...
const AllRefundOrders = () => {
  ...
  const eligibleOrders = orders && orders.filter((item) => item.status === "Đang xử lý refund");
    ...
    <div className="w-full px-5">
      ...
      <div className="w-full">
        ...
        >
          ...
          <div className=" w-[100%] 800px:w-[50%] mt-2">
            <label className="block pb-2">Nhập lại mật khẩu</label>
            <input
              type="password"
              className={`${styles.input} !w-[95%] mb-4 800px:mb-0`}
              required
...
const Address = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (addressType === "" || country === "" || city === "") {
      toast.error("Vui lòng điền trường này!");
    } else {
      dispatch(
        updatUserAddress(
          country,
    ...
    <div className="w-full px-5">
      ...
      {user && user.addresses.length === 0 && (
        <h5 className="text-center pt-8 text-[18px]">
          Bạn không có bất kỳ địa chỉ lưu!
        </h5>
      )}
    </div>
  );
